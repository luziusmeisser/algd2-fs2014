...
//        lists.add(new ch.fhnw.algd2.kevinwieser.Heap(500)); 2nd
        lists.add(new ch.fhnw.algd2.emanuelmistretta.Heap());
...
//        lists.add(new ch.fhnw.algd2.marcogaiffi.Heap());
        lists.add(new ch.fhnw.algd2.martineberle.Heap());
	}
	...
	public void test2(){
		for (IHeap heap: lists){
			...
			while (heap.peek() != null){
				...
				String cont = feeder.getNextFromControlQueue();
				assert next.equals(cont);
			}
			assert feeder.getNextFromControlQueue() == null;
		}
	}
